...
def analyze_kline_pattern(klines):
    ...
    if not klines or len(klines) < 20:
        return {"pattern": "数据不足", "strength": 50}
...
def analyze_token(token_data):
    ...
    change_24h = token_data.get("percentChange24h", 0)
    holders_top10 = token_data.get("holdersTop10Percent", 0)
